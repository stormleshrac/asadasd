...
def dashboard():
    start_msg = '<b>🚨Uso: Envia Enlaces De Descarga (Configure Antes De Empezar , Vea El /tutorial)</b>\n'
    return start_msg
...
def createCompresing(filename,filesize,splitsize):
    ...
    return msg
def createFinishUploading(filename,datacallback=None):
    msg = '✔ ' + str(filename)+ f' Subido\n'
    if datacallback:
...
def createFileMsg(filename,files):
    ...
    if len(files)>0:
        msg= '<b>🖇Enlaces🖇</b>\n'
        for f in files:
            url = urllib.parse.unquote(f.url,encoding='utf-8', errors='replace')
            #msg+= '<a href="'+f['url']+'">🔗' + f['name'] + '🔗</a>'
            msg+= "<a href='"+url+"'>🔗"+f.file+'🔗</a>\n'
        return msg
    return ''
